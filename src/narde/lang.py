"""Dependency-free language/script detection used to route between narde checkpoints.

Adapted from laya/lang.py (Apache-2.0, Convai Innovations) — see NOTICE.md.
...
# accented ones. A state that lost its accents keeps no diacritic rate for the non-English signal to
# read, so `la`, `un`, `y`, `e`, `et`, `deux` and friends are the only evidence left.
_STOP: Dict[str, set] = {
    ...
    "es": {"el", "los", "las", "que", "por", "con", "para", "una", "es", "se", "del", "como",
           ...
           # `en-US`, `en route`, `Rio de Janeiro`), so they stay out.
    ...
    "pt": {"os", "as", "que", "em", "um", "uma", "para", "com", "não", "é", "se", "do", "da",
           ...
           # `no` is Portuguese too; it is also one of the most frequent English words, so it stays
           # out and short Portuguese states that lean on it alone are left to the diacritic rate.
    ...
    # French/Spanish/Italian/Portuguese state.
...
# Words that more than one list claims. A shared word may not name a winner by itself
# (Romanian text was reported as French that way), though it still counts toward the total of a
# language that also matched a word of its own.
...
def latin_profile(text: str) -> Dict[str, object]:
    ...
    # Italian the winner -- which is a guess dressed as a detection.
...
# Public alias matching laya's public API
detect_language = analyse


# laya/__init__.py exports this alias; keep the public surface identical.
detect_language = analyse
