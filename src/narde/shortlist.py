...
Adapted from laya/shortlist.py (Apache-2.0, Convai Innovations) — see NOTICE.md.
Choice options share one ``head_max_len`` budget, so a large label set leaves only a few tokens
per label. ``predict_shortlist`` embeds the state and each option, keeps the top k, and runs one
predict on the reduced criteria set.
"""
...
from narde.prompts import render_options, serialize_state
...
def shortlist_choice(
    ...
    *,
    ...
    once, with the query text first and then one string per option in criteria order. When
    ``k`` is at least the number of labels, every label is returned in its original order and
    ``embed_fn`` is not called. Ties keep the earlier label.
    """
...
def predict_shortlist(
    ...
    **predict_kwargs: Any,
    ...
    forwarded unchanged and does not call ``embed_fn``. The caller's ``questions`` dict is not
    mutated. The returned dict is the model result plus a ``shortlist`` entry.
    """
...
def embed_fn_from_agent(
    ...
    The callable embeds a list of strings with ``agent.tok`` and ``agent.model.encoder``. It does
    not run the decision head and does not download weights. Padding positions are excluded from
    the mean. The encoder's train/eval flag is left as the caller set it.
    """
