"""High-level inference runtime for narde System 1 decision models.

Adapted from laya/agent.py (Apache-2.0, Convai Innovations) — see NOTICE.md.
"""
...
from narde.calib import clamp_temperature, confidence_from_probs, temp_bucket
from narde.model import amp_dtype, build_model
from narde.prompts import QTYPES, build_sequence, collate_items, render_options
...
def _verify_compatibility(model: torch.nn.Module, cfg: Dict, weights: Dict[str, torch.Tensor], model_id: str):
    ...
    for prefix in required_prefixes:
        if not any(k.startswith(prefix) for k in weights.keys()):
            raise ValueError(
                f"Incompatible model weights for {model_id!r}: checkpoint is missing "
                f"'{prefix}' parameters. Expected an RL Agent decision model."
            )
    ...
    if missing_keys:
        raise ValueError(
            f"Model weights incomplete for {model_id!r}: missing {len(missing_keys)} "
            f"parameter tensors (e.g. {missing_keys[:3]})."
        )
...
class Agent:
    ...
    def __init__(
        ...
        # That is a loss for the batch sizes narde runs and can hang on some platforms.
        try:
        ...
        if rejected:
            warnings.warn(
                "narde: this checkpoint ships temperatures outside [%g, %g] which would distort "
        ...
        if fell_back_from is not None:
            print(
                "\n[narde] Warning: could not place the model on %s, so it is running on CPU.\n"
                "  Reason: %s\n"
    ...
    def system_one(self, state: Union[str, dict, list], questions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Evaluate typed questions against state in a single, parallel forward pass."""
...
def load(
    ...
    `subfolder` picks one checkpoint out of a repo that bundles several:
        ...
        narde.load("convaiinnovations/laya")                           # English (repo root)
        narde.load("convaiinnovations/laya", subfolder="multilingual")
    """
