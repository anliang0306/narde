"""Route a request to the narde checkpoint best suited to it.

Adapted from laya/router.py (Apache-2.0, Convai Innovations) — see NOTICE.md.
...
from narde.lang import analyse
...
STANDALONE_MODELS: Dict[str, str] = {
...
_TYPED_DECISION_WORKFLOWS: Dict[str, set] = {
...
class RouteDecision(dict):
    ...
    def __repr__(self):
        return "RouteDecision(model=%r, reason=%r)" % (self.get("model"), self.get("reason"))


def normalise_name(name: str) -> str:
    """Map user-facing aliases to canonical checkpoint names."""
...
class Router:
    """Lazily loads narde checkpoints and sends each request to the right one.
